...
public partial class IAMDbContext : DbContext
{
    ...
    {
        // Only configure the provider if not configured by DI.
        if (!optionsBuilder.IsConfigured)
        {
            // Prefer an environment variable (set by Docker or host). If absent, fall back to a local connection string.
            var conn = Environment.GetEnvironmentVariable("DbConnectionString")
                       ?? "Host=localhost;Port=5432;Database=IAM;Username=postgres;Password=0000";
